SELECT
distinct ItemAction FROM
    RDL00005_Monitoring_BI.dbo.f_execution_log
GO
...
SELECT
    ...
    ,el.Format
    ,CASE
        WHEN el.RequestType = 'Interactive'
        AND (
            el.Format IS NULL
        OR el.Format = 'MHTML'
        ) THEN 'Viewed in Portal'
        WHEN el.RequestType = 'Interactive'
        AND el.Format IN (
            'EXCELOPENXML',
            'PDF',
...
SELECT
    *
FROM
    [RDL00001_EnterpriseDataLanding].[JDE_BI_OPS].[V_V0101]
SELECT
    el.TimeStart AS [Execution Start]
    ,el.TimeEnd AS [Execution End]
    ,DATEDIFF(SECOND, el.TimeStart, el.TimeEnd) AS [Execution Duration (sec)]
